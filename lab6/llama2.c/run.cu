...
__global__ void matmul_kernel(float* xout, const float* x, const float* w, int n, int d) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;  // 当前线程处理的行索引
...
// 主机函数：初始化数据并调用CUDA核函数
void matmul(float* xout, float* x, float* w, int n, int d) {
    float *d_x, *d_w, *d_xout;

    // 分配设备内存
    cudaMalloc((void**)&d_x, n * sizeof(float));
    cudaMalloc((void**)&d_w, d * n * sizeof(float));
    cudaMalloc((void**)&d_xout, d * sizeof(float));
    ...
    // 计算CUDA核函数所需的块和线程数
    int blockSize = 256;  // 每个块中的线程数
    int numBlocks = (d + blockSize - 1) / blockSize;  // 总块数，确保覆盖所有d行

    // 启动CUDA核函数
    matmul_kernel<<<numBlocks, blockSize>>>(d_xout, d_x, d_w, n, d);
